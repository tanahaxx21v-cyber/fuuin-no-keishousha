...
### 2026-06-21 イベント大規模拡張（PP4規模達成）
- ✅ EVENTSを51件→301件に拡張（+250件）
- ✅ 全13仲間のStage3最終章（ヴァイス・ローガン・エルク・ミラ・ゼノの不足5件追加）
- ✅ 全13仲間のStage4（maxDaysLeft:50）・Stage5（maxDaysLeft:35）を追加（計26件）
- ✅ 全21拠点の訪問回数ベースイベント2nd/3rd/5th visit追加（計38件）
- ✅ ボス討伐後イベント5件（盗賊王・鉱王・嵐竜・森王・潮王）追加
- ✅ 日数マイルストーンイベント6件（80/70/50/40/20/10日前）追加
- ✅ 仲間コンビイベント15件以上追加（13種のペア・トリオ組み合わせ）
- ✅ 封印石獲得・三石集結・石覚醒イベント追加
- ✅ 各拠点NPC・環境フレーバーイベント多数追加
- ✅ 決戦前夜・最終送別イベントを全拠点に追加
- ✅ 追加選択肢イベント8件（winChance分岐含む）
- ✅ ビルド確認・コミット・GitHubプッシュ完了（HEAD: a95e64e）
