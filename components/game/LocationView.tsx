...
const COMPANION_LOC_LINES: Partial<Record<CompanionId, { town: string[]; relay: string[]; dungeon: string[]; castle: string[] }>> = {
  gares: {
    ...
    castle: ['これが……魔王の城か。騎士として、ここで引くわけにはいかない。', '剣を抜け。最後の戦いだ。', 'ここを超えれば、平和が戻る。死ぬ気で行くぞ。'],
  },
  liz: {
    ...
    castle: ['神よ……私たちに力を。ここが最後の場所です。', 'みんなを絶対に守る。回復は任せて！', '怖いけど……みんなと一緒なら、どこでも行ける。'],
  },
  noa: {
    ...
    castle: ['……ここで全部終わらせるんだよね。矢は全部使い切るつもりで来た。', '手が震えてる。でも、狙いは絶対に外さない。', '最後の一矢まで戦う。約束する。'],
  },
  cecil: {
    ...
    castle: ['魔王の魔力が充満している……感知できる。これほどとは想定外だった。', '理論は全て頭に入れてある。後は実行するだけよ。', 'ここを越えれば、研究の全てが報われる。'],
  },
  bram: {
    ...
    castle: ['ここが最後の戦場か！テンション上がってきた！', '魔王？強ければ強いほど燃えるってもんだ！', 'やっと本物の強敵と戦えるな！全力で行くぞ！'],
  },
  finn: {
    ...
    castle: ['……来てしまいました。もう後戻りはできないですよね。', '怖いですが……先輩と一緒なら戦えます！', 'こんな場所まで来るとは……でも逃げません！'],
  },
  vais: {
    ...
    castle: ['……ここが終点か。逃げ道なんて考えてない。初めてだな。', '昔の俺なら依頼を断ってた場所だ。でも今は違う。', '退路を断って戦うのは初めてかもしれない。悪くない感覚だ。'],
  },
  logan: {
    ...
    castle: ['……ここで死んでもいい。お前たちが生き残れば。', '俺が前に出る。絶対に後ろに通さない。', '……来たか。いよいよだな。'],
  },
  iris: {
    ...
    castle: ['魔王の魔力……感じる。でも、もう迷わない。私はここにいる。', '魔族として生まれても、選べる——私はこっちを選んだ。', '魔王に怯えていた頃の私は、もういない。'],
  },
  sig: {
    ...
    castle: ['魔王の財宝……どれだけあるんだろうなあ。', 'これを生き延びれば、一生自慢話ができるな！', '死んだら元も子もない。慎重に、でも確実に行くよ。'],
  },
  elk: {
    ...
    castle: ['獣の本能が全力で警戒を叫んでいる。行くぞ。', 'ここの魔力は濃い。体毛が逆立つ。', '強い敵がいる——だから俺が来た意味がある。'],
  },
  mira: {
    ...
    castle: ['この城には緑が一本もない……魔王は本当に生命を憎んでいるのね。', '千年を生きるエルフとして——この戦いを見届ける義務がある。', '矢は最後の一本まで残してある。ここで全部使い切る。'],
  },
  zeno: {
    ...
    castle: ['魔王よ……かつて私はあなたを恐れていた。今は違う。', '人間と並んでここに立つ日が来るとは思わなかった。', '終わりにする——この城も、この戦いも、全てを。'],
  },
}
...
export default function LocationView({
  ...
  if (aliveParty.length > 0) {
    ...
    const linesForType = COMPANION_LOC_LINES[speakerId]?.[loc.type as 'town' | 'relay' | 'dungeon' | 'castle']
  ...
  const SOLO_LINES: Partial<Record<string, { town: string[]; relay: string[]; dungeon: string[]; castle: string[] }>> = {
    _: {
      town: [
        ...
        '一人でここまで来た。……でも、これが当たり前じゃない。仲間を探そう。',
        '賑やかな声を聞いていると、誰かと笑い合いたくなる。今は急ぎだが。',
      ],
      relay: [
        'ここで一息つくか。……でも、一人旅は心細い。',
        '封印石のことを考えると、立ち止まってなどいられない。',
        '誰かいればと思うが……今は先を急ごう。',
        '静かだ。一人でいると、余計なことを考えてしまう。',
        '風だけが通り過ぎていく。急がなければ。',
        '焚き火をつけても、一人分の温もりしかない。',
      ],
      dungeon: [
        '一人でここに踏み込むのは……無謀かもしれない。でも、やるしかない。',
        '力を蓄えなければ。仲間がいればもっと楽なのだが……',
        '封印石はここにある。怖れている場合じゃない。',
        '暗い。足音が響く。……自分の鼓動まで聞こえる。',
        '背中を守ってくれる人がいない。全方位、自分でカバーするしかない。',
        '一人だからこそ、慎重に行く。死ぬわけにはいかない。',
      ],
      castle: [
        '封印石を集めなければ、ここには入れない。まだ旅は続く。',
        '……今は力を蓄える時だ。',
        'この城の前に立つ日が来るとは……。まだ準備が足りない気がする。',
        '一人でここに来たか。……それでも、諦める理由はどこにもない。',
        'ここまで来た。後は前に進むだけだ。',
        '魔王を封印する——それだけを考えて、ここまで歩いてきた。',
      ],
    },
  }
    ...
    <div className="p-3 flex flex-col gap-3">
            ...
            <div className="relative max-w-sm w-full mx-4 border-2 border-red-600 p-0" style={{ background: '#0a0404' }}>
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2 mb-1.5">
                      ...
                      <div className="flex-1 h-2 bg-gray-900 border border-gray-700 overflow-hidden">
                        <div className="h-full transition-all duration-300" style={{ width: `${pct}%`, backgroundColor: fill }} />
                      </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2.5 bg-red-800 hover:bg-red-700 border-2 border-red-500 text-white font-black text-sm"
                >
                  ⚔️ 挑む！
                </button>
                <button
      ...
      <div className={`border-2 ${typeBorder}`} style={{ background: '#08080e' }}>
        <div className="flex items-center gap-3 px-3 py-2.5" style={{ borderBottom: '1px solid #1a1a2e' }}>
          <button
            onClick={onBackToMap}
            className="text-xs font-bold text-gray-600 hover:text-gray-300 px-2 py-1 border border-[#2a2a40] hover:border-gray-500 shrink-0"
          >
            ← マップ
          </button>
                ...
                <div className="px-4 pb-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 py-3 font-black text-white disabled:opacity-40 disabled:cursor-not-allowed border-2"
        ...
        <div className="flex flex-col">
          ...
          {loc.hasInn && (() => {
            ...
            return (
              <button
                onClick={onInn}
                disabled={!canAfford}
                className="w-full px-3 py-2.5 text-left flex items-center border-b border-[#2d2000] hover:bg-[#0d0a00] disabled:opacity-40 disabled:cursor-not-allowed"
              >
          ...
          {loc.type === 'castle' && (
            gs.sealStones.length < 3 ? (
              ...
              <button
                onClick={() => setShowBossConfirm(true)}
                className="w-full px-3 py-3 text-left flex items-center border-b border-[#2d2000] hover:bg-[#1a0000]"
              >
      ...
      {onUseItem && gs.inventory.filter(i => i.qty > 0 && ['heal_hp','heal_mp','heal_both','cure_status'].includes(ITEMS[i.itemId]?.effect ?? '')).length > 0 && (
        <div className="bg-[#0c0c24] border-2 border-teal-800 overflow-hidden">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-teal-950"
          >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-indigo-950 border border-indigo-700 px-3 py-2 text-xs font-bold text-white hover:bg-indigo-900"
                    >
                      <span>🧑</span>
                      <div>
